...
Formally, if the projection function is $\mathbf{\pi}$, we can warp on the previous timestep's pixels $u_r$ onto the current timestep $\hat{u_r}$ with relative pose $p_{1,t-1}$
...
Other methods that utilize CNNs for Cross View localization were then spawned. Each paper utilized a different backbone CNN architecture, and incorporated more information about the image to make the CNN output more distinctive features. 
